import React from 'react';
import ConnectionsModal from '../../src/components/ConnectionsModal';
import { MemoryRouter } from 'react-router-dom';
import { AuthProvider } from '../../src/context/AuthContext';

describe('<ConnectionsModal />', () => {
  const profileId = 'test-uuid-123';

  beforeEach(() => {
    // Interceptores de segurança originais mantidos
    cy.intercept('GET', '**/api/notifications/**', {
      statusCode: 200,
      body: [],
    });
    cy.intercept('GET', '**/api/accounts/profile/**', {
      statusCode: 200,
      body: { id: 'me-123', username: 'tester' },
    });

    // Mocks das conexões
    cy.intercept(
      'GET',
      `**/accounts/profiles/${profileId}/connections/?type=followers`,
      {
        body: [
          {
            profile_id: '1',
            username: 'follower_1',
            display_name: 'Follower One',
          },
        ],
      }
    ).as('getFollowers');

    cy.intercept(
      'GET',
      `**/accounts/profiles/${profileId}/connections/?type=following`,
      {
        body: [
          {
            profile_id: '2',
            username: 'following_1',
            display_name: 'Following One',
          },
        ],
      }
    ).as('getFollowing');
  });

  const mountWithContext = (props) => {
    cy.mount(
      <MemoryRouter>
        <AuthProvider>
          <div className="bg-[#0F1419] h-screen">
            <ConnectionsModal {...props} />
          </div>
        </AuthProvider>
      </MemoryRouter>
    );
  };

  it('deve abrir e mostrar a lista de seguidores por padrão', () => {
    mountWithContext({
      isOpen: true,
      profileId: profileId,
      initialTab: 'followers',
      onClose: cy.stub().as('onClose'),
    });

    cy.wait('@getFollowers');

    // 1. Uso do data-cy para o modal e para garantir que o conteúdo carregou
    cy.getByData('connections-modal').should('be.visible');
    cy.getByData('connection-item-follower_1').should(
      'contain',
      'Follower One'
    );
  });

  it('deve alternar entre as abas e carregar novos dados', () => {
    mountWithContext({
      isOpen: true,
      profileId: profileId,
      initialTab: 'followers',
      onClose: () => {},
    });

    cy.wait('@getFollowers');

    // 2. Alterna abas usando data-cy em vez de texto genérico
    cy.getByData('tab-following').click();
    cy.wait('@getFollowing');

    cy.getByData('connection-item-following_1').should(
      'contain',
      'Following One'
    );
  });

  it('deve mostrar mensagem de lista vazia quando não houver conexões', () => {
    cy.intercept(
      'GET',
      `**/accounts/profiles/${profileId}/connections/?type=followers`,
      {
        body: [],
      }
    ).as('getEmpty');

    mountWithContext({
      isOpen: true,
      profileId: profileId,
      initialTab: 'followers',
      onClose: () => {},
    });

    cy.wait('@getEmpty');

    // 3. Validação robusta da mensagem de lista vazia
    cy.getByData('empty-connections-message')
      .should('be.visible')
      .and('contain', 'No connections found');
  });
});
