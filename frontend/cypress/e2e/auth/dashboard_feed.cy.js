describe('Dashboard - Feed, Scroll & Storage Fix (Robust Version)', () => {
  ...
  beforeEach(() => {
    ...
    cy.intercept('POST', '**/api/posts/**', {
      // ← Mantém /api/posts/ porque é o que o frontend chama
      statusCode: 201,
      body: {
        id: 99,
  ...
  it('1. Deve exibir posts iniciais e carregar mais via Infinite Scroll', () => {
    cy.contains('Hello from Para Você!', { timeout: 15000 }).should(
      'be.visible'
    );
    ...
    cy.contains('Conteúdo da Página 2!', { timeout: 10000 }).should(
      'be.visible'
    );
  });
  ...
  it('3. Deve alternar entre abas "Para você" e "Seguindo"', () => {
    cy.contains('Hello from Para Você!', { timeout: 10000 }).should(
      'be.visible'
    );
