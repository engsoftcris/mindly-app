describe('Mindly - Fluxo de Comentários com data-cy', () => {
  ...
  beforeEach(() => {
    ...
    cy.intercept('GET', 'https://api.giphy.com/v1/gifs/**', {
      body: fullGiphyResponse,
    }).as('giphyFetch');
    cy.intercept('GET', '/api/accounts/notifications/**', { body: [] }).as(
      'getNotifications'
    );
    cy.intercept('GET', '/api/accounts/suggested-follows/**', { body: [] }).as(
      'getSuggested'
    );
  });
  ...
  it('Deve postar um comentário com imagem do PC usando seletores data-cy', () => {
    ...
    cy.get('[data-cy="image-preview"]', { timeout: 15000 }).should(
      'be.visible'
    );
  ...
  it('Deve permitir ao dono do post deletar comentário de terceiros (Moderação)', () => {
    ...
    cy.intercept('DELETE', '/api/comments/888/', { statusCode: 204 }).as(
      'deleteComment'
    );
    ...
    cy.get('body').then(($body) => {
      if ($body.find('[data-cy="confirm-delete"]').length) {
        ...
        cy.get('button')
          .contains(/Confirm|Delete|Excluir/i)
          .click({ force: true });
      }
    });
  ...
  it('Não deve permitir postar comentário vazio...', () => {
    cy.intercept('POST', '/api/comments/', { statusCode: 201 }).as(
      'postComment'
    );
    visitAuthed('/');
    cy.get('[data-cy="comment-button"]').first().click({ force: true });
